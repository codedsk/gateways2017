...
\fancypagestyle{specialfooter}{%
  ...
  \fancyfoot[R]{Presented at Gateways 2017, University of Michigan, Ann Arbor, MI, October 23-25, 2017.
...
\lstset{ %
  ...
  belowcaptionskip=5px,              % add some space under the caption and above the code
...
% fix clever refs for listings:
\crefname{lstlisting}{listing}{listings}
\Crefname{lstlisting}{Listing}{Listings}

% setup listing captions
...
The HUBzero\textsuperscript{\textregistered} Platform has a long history of
supporting interactive simulation tools with X11-based graphical user
interfaces through the web browser. In recent years, new HTML and JavaScript
based development environments have matured and are being utilized by
researchers to help them perform research, develop simulations, clean and
...
Jupyter Notebooks and RStudio's Shiny\textsuperscript{\textregistered} and
RMarkdown\textsuperscript{\textregistered} libraries, and how this
...
trying similar setups with the RStudio's IDE and Shiny, several
Node.js\textsuperscript{\textregistered}\cite{nodejs} applications, Python's
Flask\cite{flask} microframework, and Wt\cite{wt}, a lesser known \CC based
...
able work interactively with notebooks, either new ones the user just started or
...
Tg, the application shown in \Cref{fig_tg}, is a Jupyter Notebook published on
nanoHUB.org that calculates glass transition temperatures through molecular
dynamics calculations. Under the hood, it runs a workflow that incorporates
previously published Rappture applications, jobs sent to remote compute
clusters, and local calculations. Other types of Jupyter content that can be
published on the HUBzero Platform can be seen in \Cref{fig_mdvacancy} and
\Cref{fig_crack}. Note the lack of source code and editing tools shown in these
applications. With Jupyter based applications, developers have the choice of
showing or hiding their source code when the application is published.
...
\begin{figure}[!h]
  \centering
  \includegraphics[width=3in]{rstudio_bar_charts}
  \caption{RStudio IDE being served from a HUB tool container. \cite{rstudio-hz}}
  \label{fig:rstudio}
\end{figure}

\begin{figure}[!h]
  \centering
  \includegraphics[width=3in]{incip_20170709}
  \caption{Indiana Precipitation Explorer is a Shiny web application hosted on myGeoHUB.org. \cite{incip}}
  \label{fig:incip}
\end{figure}
...
\begin{figure}[!h]
  \centering
  \includegraphics[width=3in]{spironode_composed}
  \caption{Spironode is a web application that uses Node.js to generate the graphical user interface and Python for scientific calculations.}
  \label{fig:spironode}
\end{figure}

Spironode \cite{spironode} is a Node.js and Python implementation of the
Spirograph program and has been deployed as a web application running on the
HUBzero Platform. It uses the Express web application framework and EJS
templating system to generate the HTML based graphical user interface and
handle routing of requests from the user's web browser. The \textit{science}
code of calculating the coordinates for the spirograph is performed in Python,
The Node.js based user interface calls the separate python program and
feeds it inputs from the user. The calculated spirograph coordinates are then
sent back to the user where they are rendered by JavaScript running in the
browser.

%% FIXME: cite spironode:
...
Spiroflask is a similar implementation of the Spirograph program, but instead
of being written in Node.js, it uses Python's Flask microframework to generate
the web interface, and the core science code is also written in Python.

Another, lesser known library that has been used to build and deploy web
applications on the HUB is Wt. Wt serves the \CC community, providing
a web application framework and widgets.  Applications can be served using
FastCGI in coordination with a web server or the application can act as its own
web server. The latter is perfect for running in a tool container.


%\section{Building and Publishing Web Applications}
%
%In each of the use cases listed above, the process to publish the web application
%requires four element:
%
%\begin{enumerate}
%\item Source Code
...
%\end{enumerate}
%
%For many web applications, the graphical user interface may be a part of the
%source code. When published, a copy of the source code will be stored on the
%HUB so it can be served to others. Data can either be served from the HUB or
%accessed remotely by the application.
%
%Developers are encouraged to include a Makefile with their source code. The
%Makefile is used to help orchestrate building and installation of the
%application. In cases where there are no files to be compiled or installed, the
%Makefile can be left empty to show that no work needs to be performed. This is
%the case for many of the Jupyter based applications, where the notebook and
%support files can be uploaded and run in place without modification.
%
%The \textit{invoke} script is used, by the HUB, to launch the web application
%and any other supporting applications. In most cases, the \textit{invoke}
%script can be built by calling \textit{/usr/bin/invoke_app}, a script that
%resides in the tool container and does most of he heavy lifting to properly
%setup the container to run both X11 desktop and web applications.
%
%Jupyter content being published on the HUB can use the \textit{start\_jupyter}
%command to launch a web server and load the content from disk. The example
%\textit{invoke} script shown in \Cref{lst:jupyter_invoke} demonstrates using
%the \textit{-u} flag to load an Anaconda Python environment and the \textit{-C}
%flag to launch the Jupyter Notebook.
%
%\begin{xcode}{%
%  language=Bash,%
%  label=lst:jupyter_invoke,%
%  caption={Invoke scripts for Jupyter Notebooks call the \textit{start\_jupyter} command to launch a web server and load the notebook}%
%}
%#!/bin/sh
%
...
%\end{xcode}
%
%RStudio based documents and applications can use the \textit{wraprun} program to launch a web server and load content.
%
%%% FIXME: cite wrapprun
...
can now support these developers in the same way we have supported developers
of desktop applications. Furthermore, users of web development environments
...
new development environments, allowing users to publish not just static HTML
web pages, but fully interactive web application stacks, that can be published
and shared with users around the world.  As new technologies rise, the HUBzero
team continues to leverage its unique and flexible platform to serve as a
resource where people can develop, publish, and share their online simulations.
...
\bibitem{nodejs}
"Node.jsÂ: a JavaScript runtime built on Chrome's V8 JavaScript engine", \url{https://nodejs.org/en/}

\bibitem{flask}
"Flask (A Python Microframework)", \url{http://flask.pocoo.org/}
...
\bibitem{spironode}
Derrick Kearney (2017), "Spirograph in NodeJs (with Python core)," https://help.hubzero.org/resources/spironode1.

\bibitem{incip}
Derrick Kearney (2017), "Indiana Precipitation Explorer," https://mygeohub.org/resources/incip.

\bibitem{rstudio-hz}
MyGeoHUB.org (2017), "RStudio," https://mygeohub.org/resources/rstudio.
