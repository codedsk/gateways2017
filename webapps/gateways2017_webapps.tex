...
\begin{figure}[!h]
	\centering
	\subfloat[Before]{\includegraphics[width=1in]{arch_old}%
		\label{fig_first_case}}
	\subfloat[After]{\includegraphics[width=1in]{arch_new}%
		\label{fig_second_case}}
	\caption{Architecture Changes}
	\label{fig_arch}
\end{figure}

Figure ~\ref{fig_arch} shows the small number of changes actually required to
integrate web applications. On the HUB, when a user starts the a web application, the request is routed
from the user's web browser, through the HUB's webserver, and proxy to an execution host.  From there, an exec proxy sends data to and from  a tool container where a Jupyter, RStudio or other server waits to respond.  Tool containers may run simulation code or simply control simulations run on clusters or grids.
